"""
Transfer Learning Template — ConvNeXt Family
=============================================
ConvNeXt Tiny / Small / Base / Large ကိုသုံးပြီး transfer learning လုပ်မယ်။
...
# ConvNeXt variant ရွေးပါ: "convnext_tiny", "convnext_small", "convnext_base", "convnext_large"
...
# ============================
# ConvNeXt variant → (model_fn, weights)
# ============================
CONVNEXT_VARIANTS = {
...
# ============================
# Model — ConvNeXt Family
# ============================
def create_model(num_classes, variant=MODEL_NAME):
    model_fn, weights = CONVNEXT_VARIANTS[variant]
    ...
    # ConvNeXt classifier: model.classifier = [LayerNorm2d, Flatten, Linear]
    # classifier[-1] က final Linear layer
...
def train_classifier_only(model, train_loader, val_loader):
    ...
    # classifier[2] ကို ပြောင်းထားတာဖြစ်လို့ classifier parameters ယူမယ်
    classifier_params = model.classifier[2].parameters()
    optimizer = optim.Adam(classifier_params, lr=LR_CLASSIFIER)
...
def finetune_full(model, train_loader, val_loader):
    ...
    optimizer = optim.Adam([
        {"params": [p for n, p in model.named_parameters() if "classifier.2" not in n], "lr": LR_FINETUNE / 10},
        {"params": model.classifier[2].parameters(), "lr": LR_FINETUNE},
    ])
    scheduler = optim.lr_scheduler.ReduceLROnPlateau(optimizer, factor=0.5, patience=2)
